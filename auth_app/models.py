...
def default_expires_at():
    # tuỳ policy của bạn: 30 ngày kể từ bây giờ là ví dụ
    return timezone.now() + timedelta(days=30)
class UserSession(models.Model):
    '''Track active sessions for session management'''
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    session_token = models.CharField(max_length=255, unique=True, db_index=True, default=generate_session_token)
    ...
    expires_at = models.DateTimeField(db_index=True,default=default_expires_at,)
...
class AuthToken(models.Model):
    ...
    expires_at = models.DateTimeField(
        ...
        help_text="Token expiration time (defaults to 24 hours from creation)"
    )
    ...
    @staticmethod
    def create_token(user, purpose, hours=1):
        AuthToken.objects.filter(user=user, purpose=purpose).delete()
