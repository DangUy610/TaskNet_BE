...
class CardAssignee(models.Model):
    card = models.ForeignKey('boards.Card', on_delete=models.CASCADE, related_name='assignees_through')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='card_assignees')
    assigned_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='card_assignees_made')
    ...
    def clean(self):
        board = self.card.list.board if self.card.list else None
        if not board:
            raise ValidationError("Card chưa thuộc board hợp lệ.")
        if not board.is_member(self.user):
            raise ValidationError("User không phải thành viên board.")

    def save(self, *args, **kwargs):
        self.full_clean()
        return super().save(*args, **kwargs)

class CardWatcher(models.Model):
    card = models.ForeignKey('boards.Card', on_delete=models.CASCADE, related_name='watchers_through')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='card_watchers')
    enabled_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='card_watches_made')
